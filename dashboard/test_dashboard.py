...
def test_required_columns():
    ...
    assert needed.issubset(set(viz.DF.columns))
    assert len(viz.ORDERED_METABOLITES) > 0
    assert all(m in viz.DF.columns for m in viz.METABOLITES)


# 3. The cohort filter actually filters by trajectory AND sex.
def test_filter_df():
    f = viz.filter_df(["MCI->AD"], "Female")
    assert len(f) > 0
...
def test_all_rids_consistent():
    rids = viz.all_rids(["MCI->AD"], "All")
    assert len(rids) > 0
    sub = viz.DF[viz.DF["RID"].isin(rids)]
...
def test_trajectory_figure():
    fig = viz.trajectory_figure("GLCA", ["MCI->AD"], "All", False)
...
def test_forest_figure():
    fig = viz.forest_figure("MCI->AD", "stable_NL", "All", 0)
...
def test_drilldown_figure():
    rid = int(viz.DF["RID"].iloc[0])
    fig = viz.drilldown_figure(rid)
    assert len(fig.data) >= 1
    assert viz.drilldown_figure(None) is not None
...
def test_age_at_conversion_respects_filter():
    full = viz.age_at_conversion_figure(None, "All")
    one = viz.age_at_conversion_figure(["MCI->AD"], "All")
...
def test_empty_selection_semantics():
    # Tab B: empty on []
    traj = viz.trajectory_figure("GLCA", [], "All", False)
    assert len(traj.data) == 0

    # Tab D: [] shows the same number of points as selecting every class
    all_groups = list(viz.DF["trajectory"].dropna().unique())
    pts = lambda f: sum(len(t.x) for t in f.data if t.x is not None)
    assert pts(viz.pca_figure([], "All", "trajectory", False)) > 0
    assert pts(viz.pca_figure([], "All", "trajectory", False)) == \
        pts(viz.pca_figure(all_groups, "All", "trajectory", False))

    # Tab A: [] matches the unfiltered observation count
    obs = lambda f: sum(len(t.y) for t in f.data if t.y is not None)
    assert obs(viz.age_at_conversion_figure([], "All")) == \
        obs(viz.age_at_conversion_figure(None, "All"))
