...
# --------------------------------------------------------------------------- #
# Dashboard-wide NL/AD colours with a colourblind-friendlier, higher-contrast pair:
...
# App init
# --------------------------------------------------------------------------- #
# React 18 is the Dash 3.x default. If graph click events (PCA dot -> patient
# drill-down) ever misbehave under React 18, pin the Dash 2.x React BEFORE the
# app is constructed:
#   import dash
#   dash._dash_renderer._set_react_version("16.14.0")
#
# suppress_callback_exceptions: tab content (pca-fig, drilldown-fig,
# trajectory-fig, forest-fig) is rendered lazily by render_tab(), so those IDs
# are absent from the initial layout; the flag lets Dash wire the callbacks
# that reference them (without it the PCA->patient click-to-drill never fires).
app = Dash(__name__, update_title=None, suppress_callback_exceptions=True)
app.title = "Metabolite Trajectories - Visit-Ordinal"
server = app.server
...
def update_rid_options(groups, sex, current):
    ...
    options = [{"label": f"{r}  ({traj_by_rid[r]})", "value": r}
               for r in rids]
    # Default to the first RID in the list so Tab E shows a patient on load
    # (keep the current pick if one already exists, e.g. from a click).
    value = current if current is not None else (rids[0] if rids else None)
